...
- 将 shell 温控守护进程、温度采集和 LuCI 后端控制合并为 Rust 二进制 `airpi-fanctl`
- 保留旧命令路径兼容入口，并统一传感器发现、驱动选择和 PWM 写入逻辑
- 增加控制参数校验，修复手动模式与 procd 守护进程之间的写入竞争
- 保留 AP3000M MT7981 直接导出的 pwmchip 硬件 PWM 回退路径及其 0..255 占空比语义
